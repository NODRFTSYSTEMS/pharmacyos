...
Purpose: clean manual-upload checklist for the `NODRFTSYSTEMS` GitHub repository when using the GitHub web interface instead of `git push`.
...
2. Confirm these root-level items are present before upload:
   ...
   - `robots.txt`
   - `sitemap.xml`
3. In GitHub, open the repository root for `NODRFTSYSTEMS/NODRFTSYSTEMS`.
...
9. In the GitHub upload diff, confirm the pricing changes are visible:
   - `Conversion Landing Page Sprint` shows `$2,500`
   - `Static Business Site` shows `$3,500`
   - `Business Launch Site` shows `$5,500`
   - `Authority Website` shows `$12,000`
- `Platform Starter` shows `from $25,000`
- `Ecosystem Build` shows `from $50,000`
   - `Standard` retainer shows `$1,200 / mo`
...
12. Recheck the live page at `https://NODRFTSYSTEMS.github.io/NODRFTSYSTEMS/#/capabilities`.
