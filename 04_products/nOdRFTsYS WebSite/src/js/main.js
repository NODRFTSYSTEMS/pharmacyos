...
function initNavScrollState() {
  ...
  if (!nav) return;
  const update = () => nav.classList.toggle('is-scrolled', window.scrollY > 4);
  ...
  document.querySelectorAll('.js-form').forEach(form => {
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        // Send as URL-encoded — works with Google Apps Script (e.parameter)
        // and Formspree. Avoids CORS preflight that multipart/form-data triggers.
        const payload = new URLSearchParams(formData);
        payload.set('form-kind', form.dataset.formKind || '');
