 ...
 * Updates: Hourly | Source: Frankfurter v2 API
 */

const CurrencyEngine = {
  config: {
    apiEndpoint: 'https://api.frankfurter.dev/v2/rates?base=USD&quotes=COP',
  ...
  async fetchLiveRate() {
    try {
      ...
      const rate = record.rate ?? record.rates?.COP;

      if (!rate || rate < 3000 || rate > 6000) throw new Error('Rate out of expected range');
