...
/* ------------------------------------------------------------------
 * Professional Report Export — PDF-quality print view
 * Authority: DSS · SCA · TVA
 * ------------------------------------------------------------------
 * Surpasses Deal Underwriter Pro v4.2 report panel:
 *   • Executive summary with grade badge + deal score
 *   • ARV visualization with comp quality bars
 *   • Strategy-specific metrics per tab
 *   • Stress scenario bar chart
 *   • Kill switch impact table
 *   • Full comp table with quality scores
 *   • Recommendation with flag icons
...
function gradeColor(grade: string): string {
  switch (grade) {
    ...
    case "B": return "#e9a015";
    case "C": return "#f59e0b";
    case "D": return "#f43f5e";
    default: return "#6b7892";
  }
}
...
function gradeBg(grade: string): string {
  switch (grade) {
    ...
    case "B": return "rgba(233,160,21,0.12)";
    case "C": return "rgba(245,158,11,0.12)";
    case "D": return "rgba(244,63,94,0.12)";
    default: return "rgba(107,120,146,0.12)";
  }
}
...
export default function ProfessionalReportPage() {
  ...
  useEffect(() => {
    ...
    if (isDevId) {
      ...
      if (stored) { try { setAnalysis(JSON.parse(stored)); } catch { /* noop */ } }
      setLoading(false);
      return;
    }
  ...
  if (loading) return <div style={{ padding: 60, fontFamily: "Georgia, serif" }}>Loading report…</div>;
  if (!analysis || analysis.analysisUnavailable) return <div style={{ padding: 60, fontFamily: "Georgia, serif" }}>Analysis not available.</div>;

  const io = analysis.investorOutputs;
  const isAdvanced = analysis.isAdvanced;
  const today = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });

  const stressChartData = (io.stressScenarios ?? []).map((s) => ({
    label: s.label.replace(" ", "\n"),
    value: s.profit,
    color: s.status === "viable" ? "#22c55e" : s.status === "marginal" ? "#f59e0b" : "#f43f5e",
  }));
    ...
    <>
      <style>{`
        @media print {
          ...
          body { -webkit-print-color-adjust: exact; print-color-adjust: exact; background: #fff !important; color: #1a1a1a !important; }
        ...
        .report-meta { font-family: "Arial", sans-serif; font-size: 9pt; color: #888; margin-top: 12px; display: flex; gap: 24px; flex-wrap: wrap; }
        ...
        .green { color: #27ae60; }
        .amber { color: #d68910; }
        .grade-badge { display: inline-flex; align-items: center; justify-content: center; font-family: "Arial", sans-serif; font-size: 18pt; font-weight: 700; width: 44px; height: 44px; border-radius: 8px; }
      ...
      <div className="no-print" style={{ background: "#1a1a1a", padding: "12px 24px", display: "flex", gap: 12, alignItems: "center" }}>
        ...
        <button onClick={() => window.print()} style={{ marginLeft: "auto", background: "#b8962e", color: "#fff", border: "none", borderRadius: 4, padding: "8px 20px", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600, fontFamily: "Arial, sans-serif" }}>
          Print / Save PDF
        </button>
        <button onClick={() => window.history.back()} style={{ background: "transparent", color: "#888", border: "1px solid #444", borderRadius: 4, padding: "8px 16px", cursor: "pointer", fontSize: "0.82rem", fontFamily: "Arial, sans-serif" }}>
          ← Back
        </button>
      </div>
      ...
      <div className="report-body">
          ...
          <div className="report-meta">
            ...
            {io.dealGrade && (
              <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                Grade:
                <span className="grade-badge" style={{ background: gradeBg(io.dealGrade), color: gradeColor(io.dealGrade), fontSize: "10pt", width: 28, height: 28, borderRadius: 6 }}>
                  {io.dealGrade}
                </span>
                <span>({io.dealScore}/100)</span>
              </span>
            )}
          </div>
        </div>
        ...
        <div className="section">
          <div className="section-title">00 — Executive Summary</div>
        ...
        {/* Core Underwriting */}
        <div className="section">
          <div className="section-title">01 — Core Underwriting</div>
          <table className="data-grid">
            <tbody>
              <tr><td>Maximum Allowable Offer (MAO)</td><td className="gold">${io.mao.toLocaleString()}</td></tr>
              <tr><td>70% Rule MAO</td><td>${io.seventyPercentMao.toLocaleString()}</td></tr>
        ...
        {/* Stress Test Chart (Advanced) */}
        {isAdvanced && stressChartData.length > 0 && (
          <div className="section">
            <div className="section-title">02 — Stress Scenario Profit Chart</div>
            <div style={{ maxWidth: 500 }}>
              <BarChart data={stressChartData} width={480} height={160} showValues formatValue={(v) => `$${v.toLocaleString()}`} />
            </div>
          </div>
        )}
        ...
        {isAdvanced && (
          <div className="section">
            <div className="section-title">03 — Advanced Metrics</div>
            <table className="data-grid">
              <tbody>
        ...
        {isAdvanced && io.stressScenarios && io.stressScenarios.length > 0 && (
          <div className="section">
            <div className="section-title">04 — Stress Test (5 Scenarios)</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8 }}>
              {io.stressScenarios.map((s) => (
                <div key={s.label} className="stress-card">
                  ...
                  <div style={{ marginTop: 8, fontSize: "7.5pt", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }} className={s.status}>{s.status}</div>
                  <div style={{ marginTop: 4, fontSize: "7pt", color: "#aaa", lineHeight: 1.4 }}>
        ...
        <div className="section page-break">
          <div className="section-title">05 — Property Facts (Rentcast)</div>
          <div className="two-col">
            <table className="data-grid">
              <tbody>
        ...
        <div className="section">
          ...
          {analysis.triggers.length > 0 && (
            <div style={{ marginTop: 14 }}>
              <div style={{ fontSize: "8.5pt", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#888", marginBottom: 8 }}>Triage Flags</div>
